// Database Service - Uses global supabase from CDN

/**
 * Obtener comentarios de una ciudad
 */
window.getComments = async function getComments(city) {
  try {
 ...
 * Crear un comentario
 */
window.addComment = async function addComment(city, comment, userId) {
  try {
    const supabase = window.supabaseClient;
    const { data, error } = await supabase
      .from('comments')
